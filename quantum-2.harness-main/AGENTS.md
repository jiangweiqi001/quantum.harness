...
## SCNet Formal Run Contract

For `tracks/ed/hubbard-pump` Rice–Mele production runs, use
`scripts/run_rice_mele_scnet.sh` and the local SCNet profile rooted at
`/work/home/mazhuijing/ed-project`. Each run needs an immutable
`rice-mele-...` ID: stage only the allowlisted runner, test, and Slurm files to
`runs/<run-id>/source/`, preserve source SHA-256 records, and never bulk-sync
prior results. Run the resource guardrail, remote import/tests, and
`sbatch --test-only` before any submission; do not perform production ED on a
login node. Batch output must be run-scoped. Monitor through final
`COMPLETED/0:0`, fetch only to a fresh local `results/<run-id>/` directory with
non-clobbering transfer, then require checksum, manifest, eigensystem-shape,
parameter, and Hermiticity validation before treating a result as valid.
